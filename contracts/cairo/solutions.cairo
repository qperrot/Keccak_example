...
func exercise1{
    ...
    assert data_uint[2] = c_uint256;
    assert data_uint[3] = d_uint256;

    // Compute the hash
    let (hash) = keccak_uint256s_bigend{keccak_ptr=keccak_ptr}(n_elements=4, elements=data_uint);
...
func exercise2{
    ...
    assert data_uint[0] = a_uint256;
    assert data_uint[1] = b_uint256;
    assert data_uint[2] = padded_address;

    let (signable_bytes) = alloc();
    let signable_bytes_start = signable_bytes;
    keccak_add_uint256s{inputs=signable_bytes}(n_elements=3, elements=data_uint, bigend=TRUE);

    // Compute the hash
    let (hash) = keccak_bigend{keccak_ptr=keccak_ptr}(
        inputs=signable_bytes_start, n_bytes=2 * 32 + 20
    );
...
func exercise3{
    ...
    let (address_uint256, b_uint256) = _fill_bytes_array(address_uint256, b_uint256, 20);
    let (b_uint256, c_uint256) = _fill_bytes_array(b_uint256, c_uint256, 20);

    // Do right padding if address len is less than 32
    let (padded_c_uint256) = pad_right(c_uint256, 20);

    let (data_uint: Uint256*) = alloc();
    assert data_uint[0] = a_uint256;
    assert data_uint[1] = address_uint256;
    assert data_uint[2] = b_uint256;
    assert data_uint[3] = padded_c_uint256;

    let (signable_bytes) = alloc();
    let signable_bytes_start = signable_bytes;
    keccak_add_uint256s{inputs=signable_bytes}(n_elements=4, elements=data_uint, bigend=TRUE);
